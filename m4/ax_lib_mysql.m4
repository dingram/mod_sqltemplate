...
[
    AC_ARG_ENABLE(
            mysql-reentrant,
            [AC_HELP_STRING([--disable-mysql-reentrant],[Compile with libmysql rather than libmysql_r])],
            ,
            enable_mysqlreentrant=yes
        )
    ...
    if test "$want_mysql" = "yes"; then
        ...
        if test "$MYSQL_CONFIG" != "no"; then
            ...
            AC_MSG_CHECKING([if we should link against re-entrant version of MySQL])
            if test "x$enable_mysqlreentrant" = "xyes"; then
