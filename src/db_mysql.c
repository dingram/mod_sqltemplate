...
struct msqlt_db_prepared_t {
    MYSQL_STMT* stmt;
    int nargs;
    int nvals;
    msqlt_db_type_e *types;
};

struct msqlt_db_transaction_t {
    int mode;
    int errnum;
    msqlt_db_t *handle;
};
...
static apr_bucket *apr_bucket_lob_make(apr_bucket *b,
                                       const msqlt_db_row_t *row, int col,
                                       apr_off_t offset, apr_size_t len,
                                       apr_pool_t *p);
static apr_bucket *apr_bucket_lob_create(const msqlt_db_row_t *row, int col,
                                         apr_off_t offset,
                                    ...
                                    apr_size_t *len, apr_read_type_e block)
{
    ...
    const msqlt_db_row_t *row = a->row;
    msqlt_db_results_t *res = row->res;
    int col = a->col;
    apr_bucket *b = NULL;
    int rv;
...
static int dbd_mysql_select(apr_pool_t *pool, msqlt_db_t *sql,
                            msqlt_db_results_t **results,
                            const char *query, int seek)
{
    ...
    if (!ret) {
        if (sz = mysql_field_count(sql->conn), sz > 0) {
            if (!*results) {
                *results = apr_palloc(pool, sizeof(msqlt_db_results_t));
            }
            (*results)->random = seek;
...
static const char *dbd_mysql_get_name(const msqlt_db_results_t *res, int n)
{
...
static int dbd_mysql_get_row(apr_pool_t *pool, msqlt_db_results_t *res,
                             msqlt_db_row_t **row, int rownum)
{
    MYSQL_ROW r = NULL;
    int ret = 0;
    ...
    if (ret == 0) {
        if (!*row) {
            *row = apr_palloc(pool, sizeof(msqlt_db_row_t));
        }
        (*row)->row = r;
        (*row)->res = res;
...
static int dbd_mysql_get_entry(const msqlt_db_row_t *row, int n,
                               msqlt_db_datum_t *val)
{
    MYSQL_BIND *bind;
...
static const char *dbd_mysql_get_entry(const msqlt_db_row_t *row, int n)
{
    MYSQL_BIND *bind;
    if (row->res->statement) {
        bind = &row->res->bind[n];
...
static apr_status_t dbd_mysql_datum_get(const msqlt_db_row_t *row, int n,
...
static const char *dbd_mysql_error(msqlt_db_t *sql, int n)
{
    return mysql_error(sql->conn);
}

static int dbd_mysql_query(msqlt_db_t *sql, int *nrows, const char *query)
{
    int ret;
    if (sql->trans && sql->trans->errnum) {
        return sql->trans->errnum;
...
static int dbd_mysql_prepare(apr_pool_t *pool, msqlt_db_t *sql,
                             const char *query, const char *label,
                             int nargs, int nvals, msqlt_db_type_e *types,
                             msqlt_db_prepared_t **statement)
{
    /* Translate from msqlt_db to native query format */
    int ret;

    if (!*statement) {
        *statement = apr_palloc(pool, sizeof(msqlt_db_prepared_t));
    }
...
static void dbd_mysql_bind(msqlt_db_prepared_t *statement,
...
static int dbd_mysql_pquery_internal(apr_pool_t *pool, msqlt_db_t *sql,
                                     int *nrows, msqlt_db_prepared_t *statement,
                                     MYSQL_BIND *bind)
{
    int ret;
...
static int dbd_mysql_pquery(apr_pool_t *pool, msqlt_db_t *sql,
                            int *nrows, msqlt_db_prepared_t *statement,
                            const char **values)
{
    MYSQL_BIND *bind;
...
static int dbd_mysql_pvquery(apr_pool_t *pool, msqlt_db_t *sql, int *nrows,
                             msqlt_db_prepared_t *statement, va_list args)
{
    const char **values;
    int i;
...
static int dbd_mysql_pselect_internal(apr_pool_t *pool, msqlt_db_t *sql,
                                      msqlt_db_results_t **res,
                                      msqlt_db_prepared_t *statement,
                                      int random, MYSQL_BIND *bind)
{
    ...
    if (ret == 0) {
        ...
        if (!ret) {
            if (!*res) {
                *res = apr_pcalloc(pool, sizeof(msqlt_db_results_t));
            }
            (*res)->random = random;
...
static int dbd_mysql_pselect(apr_pool_t *pool, msqlt_db_t *sql,
                             msqlt_db_results_t **res,
...
static int dbd_mysql_pvselect(apr_pool_t *pool, msqlt_db_t *sql,
                              msqlt_db_results_t **res,
                              msqlt_db_prepared_t *statement, int random,
                              va_list args)
{
...
static void dbd_mysql_bbind(apr_pool_t *pool, msqlt_db_prepared_t *statement,
                            const void **values, MYSQL_BIND *bind)
{
    void *arg;
    int i, j;
    msqlt_db_type_e type;
...
static int dbd_mysql_pbquery(apr_pool_t *pool, msqlt_db_t *sql,
                             int *nrows, msqlt_db_prepared_t *statement,
                             const void **values)
{
    MYSQL_BIND *bind;
...
static int dbd_mysql_pvbquery(apr_pool_t *pool, msqlt_db_t *sql, int *nrows,
                              msqlt_db_prepared_t *statement, va_list args)
{
    const void **values;
    int i;
...
static int dbd_mysql_pbselect(apr_pool_t *pool, msqlt_db_t *sql,
                              msqlt_db_results_t **res,
                              msqlt_db_prepared_t *statement, int random,
                              const void **args)
{
...
static int dbd_mysql_pvbselect(apr_pool_t *pool, msqlt_db_t *sql,
                               msqlt_db_results_t **res,
                               msqlt_db_prepared_t *statement, int random,
                               va_list args)
{
...
static int dbd_mysql_end_transaction(msqlt_db_transaction_t *trans)
{
    int ret = -1;
    if (trans) {
...
static int dbd_mysql_transaction(apr_pool_t *pool, msqlt_db_t *handle,
                                 msqlt_db_transaction_t **trans)
{
    ...
    if (!*trans) {
        *trans = apr_pcalloc(pool, sizeof(msqlt_db_transaction_t));
    }
...
static int dbd_mysql_transaction_mode_get(msqlt_db_transaction_t *trans)
{
    if (!trans)
...
static int dbd_mysql_transaction_mode_set(msqlt_db_transaction_t *trans,
                                          int mode)
{
    if (!trans)
        return APR_DBD_TRANSACTION_COMMIT;
...
static msqlt_db_t *dbd_mysql_open(apr_pool_t *pool, const char *params,
                                 const char **error)
{
    ...
    unsigned int port = 0;
    msqlt_db_t *sql = apr_pcalloc(pool, sizeof(msqlt_db_t));
    sql->fldsz = FIELDSIZE;
...
static apr_status_t dbd_mysql_close(msqlt_db_t *handle)
{
...
static int dbd_mysql_select_db(apr_pool_t *pool, msqlt_db_t* handle,
                               const char* name)
{
    return mysql_select_db(handle->conn, name);
}

static void *dbd_mysql_native(msqlt_db_t *handle)
{
    return handle->conn;
}

static int dbd_mysql_num_cols(msqlt_db_results_t *res)
{
    if (res->statement) {
...
static int dbd_mysql_num_tuples(msqlt_db_results_t *res)
{
    if (res->random) {
        if (res->statement) {
...
APU_MODULE_DECLARE_DATA const msqlt_db_driver_t msqlt_db_mysql_driver = {
    "mysql",
    dbd_mysql_init,
    dbd_mysql_native,
    dbd_mysql_open,
